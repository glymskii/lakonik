                ...
                if let a = active {
                    Section {
                        ...
                        if !a.hasMicAudio && a.durationSec > 5 {
                            Text("Микрофон не пишется. Во время звонка iOS не отдаёт микрофон другим приложениям — в записи будут только собеседники. Если звонка нет, остановите трансляцию и запустите снова, включив микрофон в диалоге (значок 🎙).")
                        } else {
                    ...
                    Section {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            step(2, "Включите микрофон в диалоге (значок 🎙) и нажмите «Начать трансляцию»")
                        ...
                        Text("Звук собеседников берётся из приложения встречи; экран не сохраняется — только звук. Ограничение iOS: пока в Meet, Zoom или Teams идёт звонок, микрофон другим приложениям не выдаётся — ваш голос в запись не попадёт, собеседники запишутся. Вебинары и лекции, где вы слушаете, пишутся целиком. Чтобы записать встречу целиком, включите запись в самом Meet/Zoom и импортируйте файл в приложение. Обычные звонки и FaceTime записать нельзя.")
                    }
