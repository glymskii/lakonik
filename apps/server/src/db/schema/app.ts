...
export const meetings = pgTable(
  "meetings",
  {
    ...
    organizationId: uuid("organization_id").references(() => organizations.id, { onDelete: "cascade" }),
    /** Прежний владелец (id пользователя), если встреча передана при удалении участника или его аккаунта; без FK — переживает удаление аккаунта */
    orphanedFrom: text("orphaned_from"),
