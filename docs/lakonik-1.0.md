...
- **Фаза 0 — сделано 22.09.2026**: переименование, домены `lakonik.app` / `api.lakonik.app`, сайт с политиками и AASA, String Catalog, Sentry/Amplitude (ключи не заданы), экспорт DOCX/PDF починен.
- **Фаза 1 — сделано 23.09.2026 (сервер + iOS, локально проверено сквозными сценариями)**: схема организаций (`organizations`, `members`, `invitations`, `organization_domains`, `organization_id` в данных, `orphaned_from`), перенос одиночного контура при старте (`LEGACY_ORG_*`, идемпотентно), скоуп API по `X-Organization-Id` (без заголовка — пространство по умолчанию, старые сборки работают), API организаций (создание, ссылка и приглашения по почте, роли, удаление с передачей встреч, выход, передача владения, удаление, автовступление по домену, `/api/join/{token}`), `DELETE /api/me`, личное пространство и автовступление при регистрации; iOS — переключатель пространств, онбординг, карточка организации, вступление по Universal Link, удаление аккаунта. **Не сделано**: Google-вход в iOS (нужен OAuth client ID от владельца).
- **Фаза 2 — сделано (сервер + iOS; проверка подписей Apple ждёт реальных покупок)**: таблица `subscriptions`, тарифы и квоты (402 с кодами), `GET /api/billing/entitlement`, приём JWS и уведомлений App Store, суточная сверка (при ключах App Store Server API), модель отчёта по тарифу, CLI `scripts/admin.ts` (планы организаций, ручные тарифы, суперадмины); iOS — StoreKit 2, экран тарифа, paywall по причине, автостоп записи на лимите, замок на онлайн-встречах, `Lakonik.storekit` для симулятора. **Нужно от владельца**: продукты в App Store Connect (6 подписок, группа, триал 7 дней на Pro), Paid Apps Agreement, `APPSTORE_ALLOW_SANDBOX=true` на Railway на время TestFlight.
- **Фаза 4 (каталог) — сделано**: встроенный каталог из 11 нейтральных шаблонов (`packages/shared/templates.json` v2), шаблоны заказчика ADV — приватные шаблоны его организации (`templates-adv.json`, конверсия при переносе, сид на self-hosted), группы client/internal/partner/people/notes, промпт нейтрален; поддержка в приложении, статус `transcribed` со своим цветом.
- **Фаза 3 (интеграции Meet/Zoom) — сделано на сервере и в iOS, без ключей провайдеров не проверено вживую**: таблица `integrations` (токены шифруются `INTEGRATIONS_KEY`), `meetings.external_ref` для дедупликации, OAuth-подключение (`GET /api/integrations/{provider}/connect` → системное окно → `lakonik://integrations/callback`), синхронизация раз в `INTEGRATIONS_SYNC_MINUTES` (Meet REST v2 + Drive → ffmpeg → штатный импорт с проверкой квот; при отказе — push «Запись не импортирована»), вебхук Zoom с проверкой подписи и fallback-опрос, участники и транскрипт Meet как подсказки спикерам; iOS — экран «Интеграции: Meet, Zoom». **Нужно от владельца**: проект Google Cloud (OAuth client, scopes `meetings.space.readonly` + `drive.meet.readonly`, redirect `https://api.lakonik.app/api/integrations/google_meet/callback`, заявка на верификацию), приложение Zoom Marketplace (redirect `…/zoom/callback`, событие `recording.completed` на `…/zoom/webhook`, секрет), переменные `GOOGLE_INTEGRATION_*`, `ZOOM_*`, `INTEGRATIONS_KEY` на Railway. Первая синхронизация смотрит на сутки назад; импорт останавливается на `transcribed` (тип встречи выбирает пользователь).
- **Прод**: сервер на Railway пока на сборке без организаций. Деплой новой версии = перенос данных ADV в организацию при старте, для этого на Railway нужны `LEGACY_ORG_NAME=ADV Kazakhstan`, `LEGACY_ORG_OWNER_EMAIL=<почта владельца в приложении>`, `LEGACY_ORG_PLAN=enterprise`, `LEGACY_ORG_SEATS=100`, `LEGACY_ORG_UNTIL=2026-12-31` — **без них деплоить нельзя** (данные разъедутся по личным пространствам). Решение и почта владельца — за владельцем.
- **Дальше**: фаза 5 — релиз (раздел 15), Google-вход, push-relay для корпоративного контура.
