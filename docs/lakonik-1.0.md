...
- **Фаза 3 (интеграции Meet/Zoom)** — сервер в работе у агента (таблица `integrations`, OAuth, синхронизация, вебхук Zoom); iOS-экран «Интеграции» и схема `lakonik://` готовы. **Нужно от владельца**: проект Google Cloud (OAuth client, scopes Meet/Drive, верификация), приложение Zoom Marketplace, переменные `GOOGLE_INTEGRATION_*`, `ZOOM_*`, `INTEGRATIONS_KEY`.
